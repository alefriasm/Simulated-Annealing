...
// Funciones de contar capas, HS (LO AGREGUE YO)
  int GraphB::enqc(set<int>* capas, int num_capas, int node)
  {
    int cual = -1;
    for (int i = 0; i < num_capas; i++){
      if (capas[i].count(node) > 0){
        
        cual = i;

          if (cual != -1){
            //cout << "El nodo " << node << " se encuentra en la capa " << cual << endl;
          }
        break;
      }
    }
      if (cual == -1){
        //cout << "EL nodo " << node << "no se encontro en ninguna capa. " << endl;
      }
...
double GraphB::hierarchical_score(set<int>* capas, int num_capas)
{
    ...
    for (int j = 0; j < number_of_nodes(); j++){
        if (weight(i,j)){
                ...
                int capa_i = enqc(capas, num_capas, i);
                int capa_j = enqc(capas, num_capas, j);

            if (capa_i < capa_j)
                {
                    ++Nd; 
                } else if (capa_i > capa_j){
                    ++Nu; 
                } else {
                    ++Nh;
                }
...
//Calcular el Hierarchical Score
    double score = 
    static_cast<double>(Nd + Nh) /
    static_cast<double>(Nu + Nh);
    return score;


}

void GraphB::informacion_capas(std::set<int>* capas, int num_capas)
{
  for (int i = 0; i < num_capas; i++){
    cout << "Capa " << i << " : ";
    for (auto node : capas[i]){
      cout << node << " ";
    }
    cout << endl;
  }
...
double GraphB::corrected_hierarchical_score(int Nd_obs, int Nu_obs, int Nh_obs, int Nd_exp, int Nu_exp, int Nh_exp) {
   if (Nd_exp == 0 || Nu_exp == 0 || Nh_exp == 0){
        throw std::invalid_argument("Los valores esperados no pueden ser cero. ");
    }

double ratio_Nd_obs_exp = static_cast<double>(Nd_obs) / (Nd_exp);
double ratio_Nu_obs_exp = static_cast<double>(Nu_obs) / (Nu_exp);
double ratio_Nh_obs_exp = static_cast<double>(Nh_obs) / (Nh_exp);

if (ratio_Nd_obs_exp < 0 || ratio_Nu_obs_exp < 0 || ratio_Nh_obs_exp < 0) {
        throw std::logic_error("Las proporciones observadas/esperadas no pueden ser negativas.");
    }

double numerator = (ratio_Nd_obs_exp + ratio_Nh_obs_exp);
double denominator = (ratio_Nu_obs_exp + ratio_Nh_obs_exp);

     if (denominator == 0){
      return 1000000;
    }

double corrected_score = numerator / denominator;
...
int GraphB::cal_nodos_obs(const std::set<int>& capa){
  return capa.size();
}

int GraphB::cal_nodos_esp(const std::set<int>& capa){
  return capa.size();
}

void GraphB::probabilidad(set<int>* capas, int num_capas, int num_nodes, double* probabilidades){
    for (int i = 0; i < num_capas; i++){
    probabilidades[i] = capas[i].size()/(num_nodes * 1.0);
}

    cout << "Probabilidades de aparición de nodos en cada capa: " << endl;
    for (int i = 0; i < num_capas; i++){
      cout << "Nodo " << i << ": " << probabilidades[i] << endl;
      }
    }



double GraphB::probabilistic_hierarchical_score(set<int>* capas, int num_capas, int num_nodes) {
  double* probabilidades = new double[num_capas];
  probabilidad(capas, num_capas, num_nodes, probabilidades);
...
for (int i = 0; i < num_nodes; i++){
  for (int j = 0; j < num_nodes; j++){
      if (weight(i,j) != 0){
        capa_i = enqc(capas, num_capas, i);
        capa_j = enqc(capas, num_capas, j);

        probi = probabilidades[capa_i];
        probj = probabilidades[capa_j];
        ...
        if (capa_i < capa_j){
          ...
          horizontal += probi * probj;
        }
        }
      }
}
    cout << "arriba: " << arriba << ", abajo: " << abajo << ", horizontal: " << horizontal << endl;

    double numerator = abajo + horizontal;
    double denominator = arriba + horizontal;

    if (denominator == 0.0){
      return 1000000;
    }

    double phs = numerator / denominator;
    return phs;
